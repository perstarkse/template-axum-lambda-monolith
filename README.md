...
```bash
nxtpoll-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
nxtpoll-api$ sam local start-api
nxtpoll-api$ curl http://localhost:3000/
```
...
```bash
nxtpoll-api$ sam logs -n HelloWorldFunction --stack-name nxtpoll-api --tail
```
